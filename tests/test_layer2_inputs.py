...
from ML.deep_research.domain_decider import create_run
from ML.deep_research.domain_decider.backend.fs import load_json, sha256
from ML.deep_research.domain_decider.backend.runner import source_sections
from ML.deep_research.domain_decider.backend.settings import PROMPTS_DIR, PROMPT_FILES
from ML.deep_research.domain_decider.backend.windows import encoding, source_windows, token_count
...
class InputTests(unittest.TestCase):
    ...
    def test_cli_passes_explicit_confirmation_and_has_no_implicit_opt_in(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("ML.deep_research.domain_decider.backend.cli.create_run", return_value=run) as create, patch(
                "ML.deep_research.domain_decider.backend.cli.run_all"
            ), patch("ML.deep_research.domain_decider.backend.cli.load_dotenv_key"):
    ...
    def test_new_prompt_snapshots_do_not_refresh_old_ones(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("ML.deep_research.domain_decider.backend.create_run.PROMPTS_DIR", prompts):
                old = new_run(root)
