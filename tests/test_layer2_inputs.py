...
class InputTests(unittest.TestCase):
    ...
    def test_complete_catalogue_inline_or_paged_never_truncated(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(pages[0]["facts"][0]["fact_id"], "f1")
            self.assertIn("Fact: Unchanged source", pages[0]["facts"][0]["text"])
            ...
            self.assertTrue(all(p["facts"] == pages[0]["facts"] for p in pages))
            for page in pages:
    ...
    def test_repacking_preserves_source_and_fits_assembled_input(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payloads = list(source_payloads(run))
