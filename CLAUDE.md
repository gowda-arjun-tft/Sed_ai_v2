...
Read `AGENTS.md` and `CLAUDE_HANDOVER.md` when starting work in this repository. Inspect the current branch and working-tree diff before editing; the handover is a dated snapshot.

Use the personal `deep-agents-builder` skill for Python Deep Agents work and `prompt-framework` for production prompt work, including their relevant references. Prefer `deep-agents-builder` over the older personal `deepagents-builder`. Apply the repository's Model-Output Freedom contract when general skill examples differ.
