"""Read-only technical checks for Layer 2 schema version 3."""
...
from .fs import load_json, sha256, slug
...
def _load_json_or_none(path: Path) -> Any:
    """Input a path; return its JSON value or None for observational checks."""
    try:
...
def _checks(run_dir: Path, run: dict[str, Any]) -> list[Check]:
    """Input a run path and record; return technical checks without grading content."""
    ...
    checks = [
        ...
        (4, "Eight domain-context files are valid JSON", missions_ok, f"contexts={sum(isinstance(v, dict) for v in mission_values)}"),
    ]
    return checks


def run_checks(run_dir: Path) -> list[Check]:
    """Input a run path; return read-only technical checks for the CLI and tests."""
    record_path = run_dir / "run.json"
    if not record_path.is_file():
        raise FileNotFoundError(f"not a CDI run folder: {run_dir}")
    run = _load_json_or_none(record_path)
    run = run if isinstance(run, dict) else {}
    return _checks(run_dir, run)
