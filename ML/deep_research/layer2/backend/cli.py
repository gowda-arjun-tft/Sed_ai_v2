...
def _require_current_run(parser: argparse.ArgumentParser, run_dir: Path) -> None:
    """Input a CLI parser and run; stop CLI execution unless the run is schema 7."""
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Extract evidence once, design domains and assign fact IDs (Layer 2 schema 7)."
    )
    parser.add_argument("fact_sheet", nargs="?", type=Path)
    parser.add_argument("--domain-plugin", type=Path)
    parser.add_argument("--requirements", type=Path)
    parser.add_argument("--resume", type=Path, help="resume a schema-7 Layer 2 run")
    parser.add_argument("--check-only", type=Path, help="inspect a schema-7 Layer 2 run")
