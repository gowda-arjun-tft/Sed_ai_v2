...
from ..claims import context_from_claim_block
...
def write_missions(run_dir: Path) -> None:
    ...
    for definition in definitions:
        ...
        if not entries and "json input is silent" not in mission_text.casefold():
            mission_text = (
                f"The JSON input is silent on this subject. {mission_text.strip()}"
            )
        write_json(
            mission_path,
            {
                ...
                "context": [
                    (
                        context_from_claim_block(block)
                        if run["split_mode"] == "claims"
                        else context_from_block(block)
                    )
