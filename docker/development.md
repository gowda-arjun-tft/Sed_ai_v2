...
Both renamed modules retain schema 9; Docker runs the currently checked-out code.
...
python -c "from ML.deep_research.domain_decider.backend.settings import LAYER2_SCHEMA_VERSION; print(LAYER2_SCHEMA_VERSION)"
python -m pip check
```

Expected: Linux, `/usr/local/bin/python`, successful imports and Domain Decider schema **9**.
...
Then edit the [combined notebook controls](../README.md#one-notebook-execution-cell) and run its one code cell when ready;
dynamic Layer 2 schema-9 outputs feed the new Layer 3 source preparation/research workflow;
Layer 4 execution has been removed. Existing schema and frozen-path
...
`docker compose exec -T dev /usr/local/bin/python -m ML.deep_research.research_module ...` instead.
...
Tests cover the shared mount, interpreter and the single compilable notebook code cell with portable paths.
